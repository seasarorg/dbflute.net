...
namespace DBFluteRuntime.JavaLike.Util
{
     ...
     *
     * <p>The methods in this class all throw a <tt>NullReferenceException</tt> if
    ...
    public class Arrays
    {
         ...
         * @throws IndexOutOfRangeException if <tt>fromIndex &lt; 0</tt> or
        ...
        private static void sort2(float[] a, int fromIndex, int toIndex) {
            int NEG_ZERO_BITS = BitConverter.ToInt32(BitConverter.GetBytes(-0.0f), 0);
            /*
             * The sort is done in three phases to avoid the expense of using
             * NaN and -0.0 aware comparisons during the main sort.
             */
		    ...
		    float swap = a[i];
                    ...
                    if (a[i]==0 && BitConverter.ToInt32(BitConverter.GetBytes(a[i]), 0)==NEG_ZERO_BITS) {
                        a[i] = 0.0f;
                        numNegZeros++;
                    }
                    i++;
        ...
        private static void sort1(long[] x, int off, int len) {
	        ...
	        int size, index = off + len;
            size = Math.Min(a - off, b - a); vecswap(x, off, b - size, size);
            size = Math.Min(d - c, index - d - 1); vecswap(x, b, index - size, size);

	        // Recursively sort non-partition-elements
            if ((size = b - a) > 1)
                sort1(x, off, size);
            if ((size = d - c) > 1)
                sort1(x, index - size, size);
        }
         ...
         */
        private static void mergeSort<T>(T[] src,
                      T[] dest,
                      int low,
                      int high,
                      int off)
        {
            ...
            if (Length < INSERTIONSORT_THRESHOLD)
            {
                for (int i = low; i < high; i++)
                    for (int j = i; j > low &&
                 ((Comparable<T>)dest[j - 1]).compareTo(dest[j]) > 0; j--)
                        swap(dest, j, j - 1);
                return;
            }
            ...
            int mid = shiftRightPaddingZero(low + high, 1);
        ...
        //        for (int i=low; i<high; i++)
        //            for (int j=i; j>low &&
        //         ((Comparable<object>) dest[j-1]).compareTo(dest[j])>0; j--)
        //                swap(dest, j, j-1);
        //        return;
        //    }
        ...
        /// <remarks>
        /// 「>>」を使った場合、C#では符号ありの型は算術右シフト、符号なしの型の場合は論理右シフト（ゼロ埋め）が
        /// 適用される。javaコードではゼロ埋め右シフト演算子が使われていたため、
        /// 一度符号なしの型に変換してから右シフトを行っている
        /// </remarks>
        /// <returns></returns>
        private static int shiftRightPaddingZero(int before, int shiftSize)
        {
            return (int)(((uint)before) >> shiftSize);
        }
        ...
        */
         ...
         */
        public static void sort<T>(T[] a, Comparator<T> c) {
	        T[] aux = (T[])a.Clone();
            if (c==null)
                mergeSort(aux, a, 0, a.Length, 0);
            else
         ...
         *	       <tt>toIndex &gt; a.Length</tt>
         */
        public static void sort<T>(T[] a, int fromIndex, int toIndex,
				    Comparator<T> c) {
            rangeCheck(a.Length, fromIndex, toIndex);
        ...
        private static void rangeCheck(int arrayLen, int fromIndex, int toIndex) {
            if (fromIndex > toIndex)
                throw new ArgumentException("fromIndex(" + fromIndex + 
                           ") > toIndex(" + toIndex+")");
            if (fromIndex < 0)
                throw new IndexOutOfRangeException(string.Format("fromIndex={0}", fromIndex));
            if (toIndex > arrayLen)
                throw new IndexOutOfRangeException(string.Format("toIndex={0}", toIndex));
        }
	        ...
	        while (low <= high) {
	            ...
	            double midVal = a[mid];
                    ...
                    if (midVal < key) {
                        ...
                        long midBits = BitConverter.DoubleToInt64Bits(midVal);
                        long keyBits = BitConverter.DoubleToInt64Bits(key);
	        ...
	        while (low <= high) {
	            ...
	            float midVal = a[mid];
                    ...
                    if (midVal < key) {
                        ...
                        int midBits = BitConverter.ToInt32(BitConverter.GetBytes(midVal), 0);
                        int keyBits = BitConverter.ToInt32(BitConverter.GetBytes(key), 0);
                        cmp = (midBits == keyBits ?  0 : // Values are equal
                               (midBits < keyBits ? -1 : // (-0.0, 0.0) or (!NaN, NaN)
                                1));                     // (0.0, -0.0) or (NaN, !NaN)
                    }
         ...
         */
        public static int binarySearch<T, E>(T[] a, T key, Comparator<E> c) where E : T {
         ...
         * @since 1.6
         */
        public static int binarySearch<T, E>(T[] a, int fromIndex, int toIndex,
				           T key, Comparator<E> c) where E : T {
	        rangeCheck(a.Length, fromIndex, toIndex);
            return binarySearch0(a, fromIndex, toIndex, key, c);
        }

        // Like public version, but without range checks.
        private static int binarySearch0<T, E>(T[] a, int fromIndex, int toIndex,
					     T key, Comparator<E> c) where E : T {
            if (c == null) {
                return binarySearch0(a, fromIndex, toIndex, key);
	        }
	        ...
	        while (low <= high) {
                int mid = shiftRightPaddingZero(low + high, 1);
	            T midVal = a[mid];
	            int cmp = c.compare((E)midVal, (E)key);
        ...
        public static bool equals(double[] a, double[] a2) {
            ...
            for (int i=0; i<length; i++)
	        if (BitConverter.DoubleToInt64Bits(a[i]) != BitConverter.DoubleToInt64Bits(a2[i]))
                    return false;
        ...
        public static bool equals(float[] a, float[] a2) {
            ...
            for (int i=0; i<length; i++)
	        if (BitConverter.ToInt32(BitConverter.GetBytes(a[i]), 0) != BitConverter.ToInt32(BitConverter.GetBytes(a2[i]), 0))
                    return false;
         ...
         *	       <tt>toIndex &gt; a.Length</tt>
         */
        public static void fill(bool[] a, int fromIndex, int toIndex,
                                bool val) {
            rangeCheck(a.Length, fromIndex, toIndex);
            for (int i=fromIndex; i<toIndex; i++)
                a[i] = val;
         ...
         * @throws NullReferenceException if <tt>original</tt> is null
         * @since 1.6
         */
        public static T[] copyOf<T>(T[] original, int newLength) {
            return copyOf<T, T>(original, newLength, typeof(T));
        }
         ...
         * @since 1.6
         */
        public static T[] copyOf<T,U>(U[] original, int newLength, Type newType) where U : T {
         ...
         * @throws IndexOutOfRangeException if <tt>from &lt; 0</tt>
         *     or <tt>from &gt; original.Length()</tt>
         * @throws ArgumentException if <tt>from &gt; to</tt>
         * @throws NullReferenceException if <tt>original</tt> is null
         * @since 1.6
         */
        public static T[] copyOfRange<T>(T[] original, int from, int to) {
            return copyOfRange<T>(original, from, to);
        }
        ...
        // * @since 1.6
        // */
        //public static T[] copyOfRange<T,U>(U[] original, int from, int to, Type newType) where U : T {
         ...
         */
        [Serializable]
        private class ArrayList<E> : AbstractList<E>, RandomAccess
        {
